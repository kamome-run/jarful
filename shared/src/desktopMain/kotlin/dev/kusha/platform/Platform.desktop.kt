...
fun dataDirectory(): File {
    ...
    val dir = when {
        os.contains("win") -> File(System.getenv("APPDATA") ?: System.getProperty("user.home"), "Kusha")
        os.contains("mac") -> File(System.getProperty("user.home"), "Library/Application Support/Kusha")
        else -> File(System.getenv("XDG_DATA_HOME") ?: (System.getProperty("user.home") + "/.local/share"), "kusha")
    }
    dir.mkdirs()
    return dir
}
...
actual class SoundPlayer actual constructor() {
    actual fun play(pcm: ShortArray, sampleRate: Int) {
        thread(isDaemon = true, name = "kusha-sound") {
            runCatching {
