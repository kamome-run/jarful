...
| 發現自己在拖延時，寫下 **接下來的 3〜5 件事** 馬上開始 | `Ctrl+K` **重新聚焦**：一行一件 → 立刻變成票券出現在今天分頁 |
...
1. **整理例行事項**（例行事項分頁）：把早晨簡單的習慣（泡咖啡、開窗…）由上而下排好。可依星期開關；「處理 10 封郵件」這類要計數的習慣，把數量填進 **配額**。 例行事項會依 **分類標題** 分組顯示；拖曳列首的 ≡ 可排序（拖到其他標題下會一併更改分類），也可指定數量 **複製**，或 **批次選取** 後一次開啟 / 關閉。
2. **前一晚自動準備**：在設定的「準備明天票券的時間」（預設 21:00）之後開啟 App，就會自動建立明天的例行事項票券。早上開啟時若尚未產生，也會立刻產生當天的。 想在 **前一晚先印出隔天早上的票券**，請在例行事項分頁使用「選擇日期列印」並選擇明天。
3. **拆解任務**（欄位分頁）：在最左欄建立大任務（「打掃家裡」），選取後按 `Tab`（或「新增子任務」）在右欄拆成「廚房」「浴室」…，再拆到「洗碗」「擦流理台」這種 **2〜5 分鐘** 的大小。擱置超過 3 天的任務會提示「請再拆得更細」。 階層最多到 **曾孫任務**（四欄）。拖曳各列的 ≡ 可上下排序，選單的 **複製** 可連同子任務一起複製指定數量。過長的文字會自動換行。
4. **加入今天的票券**：選取任務按 `T`，整欄則按 `Shift+T`（或選單的「整欄 → 今天」）。它們會以收據樣式的卡片出現在「今天」分頁。
5. **列印並貼上**（選用）：`Ctrl+P` 列印今天全部票券，一張張撕下 **用磁鐵貼在白板上**。
6. **執行 → 完成**：做完按 **完成**（或把卡片向右滑），卡片會揉成一團掉進罐子，並有音效與震動。實體票券也撕下來揉掉丟進罐子。
7. **發現自己在拖延時**：`Ctrl+K`（⚡ 重新聚焦），把接下來要做的 3〜5 件事一行一件寫下並按 **開始**。會立刻變成票券出現在今天分頁。
...
紙張寬度為 **58 mm（384 點）** 或 **80 mm（576 點）**。 列印一律使用最大濃度（沒有設定項目）。沒有裁刀的口袋印表機請把 **Cut 關閉**，進紙行數設 3〜5 即可。
